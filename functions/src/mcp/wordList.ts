...
/**
 * Pairing-code word list: the BIP39 English wordlist (2048 curated,
 * unambiguous, non-offensive words, 3-8 letters), uppercased. Consumed by
 * mintCode() in pairing.ts to build human-readable WORD-NNNN codes.
 *
 * Curated source matters: codes are shown to university students, so the
 * list must be free of offensive or ambiguous words. BIP39 is the
 * industry standard curated for exactly this property.
 *
 * Enforced by functions/src/__tests__/wordList.test.ts (CI gate): fails
 * the build if the list drops below 2000 entries or contains a non-alpha
 * word, which resolve_session_code's /^[A-Za-z]+-\d{4}$/ regex requires.
 */
